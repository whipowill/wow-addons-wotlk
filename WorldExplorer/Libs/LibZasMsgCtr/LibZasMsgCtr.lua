...
--[[====================================================================================================================================================
	...
	HISTORY
	=============================================================================================================================================
	|CHANGED BY	| DATE			| DESCRIPTION OF CHANGE																							|
	|-------------------------------------------------------------------------------------------------------------------------------------------|
	|Zasurus	| 2010-05-08	| Added code to ensure it didn't error if the Calling Addon variable in "Msg" is not a addon table.				|
	|Zasurus	| 2010-07-04	| Changed StoreDebugMsgs initilisation to ensure it is false by default!										|
	|                           | Also added popup messages asking to disable the log if it's enabled and wipe it if it exists and isn't enabled|
	|Zasurus	| 2010-07-25	| Changed it so it uses the X-VersionType from the TOC file if it exists.										|
	=============================================================================================================================================
======================================================================================================================================================]]
...
-- Header
-----------------------------------------------------------
	local ZMC = LibStub:NewLibrary("LibZasMsgCtr-1.0", 2)
...
-----------------------------------------------------------
	...
	local Ask2DisableDebug = true; -- This is the default value so it always asks to disable the debug if it's on on load
	local Ask2WipeDebug = true; -- This is the default value so it always asks to wipe debug logs if they exist and the debug log is turned off on load
-----------------------------------------------------------
...
function ZMC:Initialize(CallingAddon, AddonName, DefaultColour1, Debug_Frame, DefaultColour2, DefaultErrorColour, DefaultMsgFrame) -- Initialize the calling addon's settings
	...
	--------------------------------------------------------------------------------
		...
		if not(StoreDebugMsgs == true) then
...
function ZMC:Msg(CallingAddon, sMessage, bDebug, bDebugOn, bError, bSimple, FrameOverride) -- Controls the displaying of messages to the user and debugging messages.
	...
	----------------------------------------------------------------------------------------
		...
		if (not((type(CallingAddon) ~= "table") or CallingAddon.ZMC_AddonName == nil)) then
			...
			if not(bSimple) then -- Addon HAS been initialized and simple mode is not wanted so use the addon's values.
				...
				local sVersionType = GetAddOnMetadata(CallingAddon.ZMC_AddonName, "X-VersionType");
				...
				-----------------------------------------------
				-- Add any ALPHA or BETA tags
				-----------------------------------------------
					if (sVersionType == "ALPHA") then -- If this is a ALPHA version then
						AddonVersion = self.COLOUR_RED.." ALPHA"..tostring(AddonVersion)..self.COLOUR_CLOSE
					elseif (sVersionType == "BETA") then -- If this is a BETA version then
						AddonVersion = self.COLOUR_ORANGE.." BETA"..tostring(AddonVersion)..self.COLOUR_CLOSE
					end;
				-----------------------------------------------
				...
				-----------------------------------------------
					if (strlen(AddonVersion) > 0) then
						AddonDetails = tostring(CallingAddon.ZMC_AddonName)..tostring(AddonVersion)..": ";
					else
	...
	--------------------------------------------------------------------------------------------
		if (CallingAddon) then
			---------------------------------------------------------
			-- Disable Debug Log Static Popup Dialog
			---------------------------------------------------------
				StaticPopupDialogs["DisableDebugLog"] = {
					text = "The debug log for "..tostring(CallingAddon.ZMC_AddonName).." is enabled. If the Author didn't tell you to enable it then you should disable and wipe it now. Do you want to disable it?",
					button1 = "Yes",
					button2 = "No",
					OnAccept = function()
						self:DisableDebugLog(tostring(CallingAddon.ZMC_AddonName)); -- Disables the Debug Log
					end,
					timeout = 0,
					whileDead = true,
					hideOnEscape = true,
			...
			---------------------------------------------------------
				StaticPopupDialogs["WipeDebugLog"] = {
					text = "The debug log for "..tostring(CallingAddon.ZMC_AddonName).." exists. If the Author didn't tell you to enable it then you should wipe it now to save memory. Do you want to wipe it?",
					button1 = "Yes",
					button2 = "No",
					OnAccept = function()
						self:WipeDebugLog(tostring(CallingAddon.ZMC_AddonName)); -- Wipes the Debug Log
					end,
					timeout = 0,
					whileDead = true,
					hideOnEscape = true,
			...
			if (CallingAddon and (type(CallingAddon) == "table") and CallingAddon.ZMC_AddonName and _G[CallingAddon.ZMC_AddonName.."_StoreDebugMsgs"] and (_G[CallingAddon.ZMC_AddonName.."_DebugLog"] ~= nil) and (type(_G[CallingAddon.ZMC_AddonName.."_DebugLog"]) == "table")) then
				...
				if CurrentDebugLogNum > MaxDebugLogEntrys then -- The array is HUGE! Stop debugging until this is deleted and inform the user
					...
					self:Msg(CallingAddon, "WARNING: The DebugLog is HUGE! To prevent errors debugging has been switched off. If you wern't asked to turn it on by an author then you should wipe this log and reduce memory usage.", nil, nil, true);
			...
			elseif (CallingAddon and (type(CallingAddon) == "table") and CallingAddon.ZMC_AddonName and (_G[CallingAddon.ZMC_AddonName.."_DebugLog"] ~= nil) and (type(_G[CallingAddon.ZMC_AddonName.."_DebugLog"]) == "table") and (#(_G[CallingAddon.ZMC_AddonName.."_DebugLog"]) > 0) and Ask2WipeDebug) then
...
function ZMC:DisableDebugLog(CallingAddon)
	...
	_G[CallingAddon.."_StoreDebugMsgs"] = false; -- Stores the local copy back over the the addon's StoreDebugMsgs boolean (It's now definatly inishilized even if it's not going to be stored!)
	self:Msg(_G[CallingAddon], "Debugging disabled");
...
function ZMC:SlashCmdHandler(msg)
	local errorMsg = "Please use enabledebuglog, disabledebuglog or wipedebuglog followed by the addon's name. E.G. /ZMC disabledebuglog MyAddon";
	
	if (msg == nil or msg == "") then
		self:Msg(nil, errorMsg);
	...
	elseif (strlower(strsub(msg,1,14)) == "disabledebuglog") then
		--self:Msg(self, "DisableDebugLog("..tostring(msg)..")");
		local CallingAddonName = strsub(msg,16)
		if (strlen(CallingAddonName) > 0) then
